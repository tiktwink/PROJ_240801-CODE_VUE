...
function useRowHighlight(props, tableRef) {
  ...
  var _useDefaultValue = hooks_useDefaultValue["default"](activeRowKeys, defaultActiveRowKeys.value, props.onActiveChange, "activeRowKeys"),
  ...
  var handleInactive = function handleInactive(ctx) {
    var row = ctx.row,
      index = ctx.index;
    var rowValue = get__default["default"](row, props.rowKey);
    if (activeRowType.value === "single") {
      ...
      for (var i = 0, len = data.value.length; i < len; i++) {
        ...
        if (_newActiveRowKeys.includes(get__default["default"](row2, props.rowKey))) {
          activeRowList.push({
            row: row2,
            rowIndex: i
          });
  ...
  var handleActive = function handleActive(ctx) {
    ...
    if (activeRowType.value === "single") {
      ...
      for (var i = 0, len = data.value.length; i < len; i++) {
        var row2 = data.value[i];
        if (newActiveRowKeys.includes(get__default["default"](row2, props.rowKey))) {
          activeRowList.push({
            row: row2,
            rowIndex: i
          });
  ...
  var handleShiftActive = function handleShiftActive(ctx) {
    ...
    var newActiveRowKeys = newActiveRowData.map(function (item) {
      return get__default["default"](item.row, props.rowKey);
    });
    setTActiveRow(newActiveRowKeys, {
  ...
  var setAllActive = function setAllActive() {
    ...
    var activeKeys = data.value.map(function (item) {
      return get__default["default"](item, props.rowKey);
    });
