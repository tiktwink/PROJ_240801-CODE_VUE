  ...
  export default {
    ...
    watch: {
      ...
      paperreferuploaddone(newValue, oldValue) {
        ...
        .then(res => {
          if (res.data.code === 0) {
            this.paper.hasUrlEmpty = 1
            this.paper.urlEmpty = res.data.data.urlEmpty
    ...
    methods: {
      ...
      importPaper() {
        ...
        this.appStore.sidePart = 'paperemptyupload'
    ...
    <div class="relative">
      <p class="font-bold text-xl mb-3 relative">查看底卷
        ...
        <svg v-if="appStore.sideOn&&appStore.sidePart==='paperemptyupload'" class="icon" height="16" p-id="15557"
          ...
          <text v-if="appStore.sideOn&&appStore.sidePart==='paperemptyupload'">取消导入</text>
      ...
      <div v-if="paper.hasUrlEmpty===1" class="flex justify-center">
        <n-scrollbar id="scroll" class="border-none cursor-pointer"
          ...
          <div class="flex justify-center items-center bg-white rounded-lg w-full">
            <img :src="'http://'+paper.urlEmpty" alt="答案图像"
      ...
      <div v-else class="flex flex-col items-center gap-y-4 pt-4">
        <p class="text-red-500 text-xl font-bold">当前底卷尚未导入图像...o_O</p>
        <span class="btn btn-ghost border-none bg-white btn-sm hover:bg-secondary font-bold " @click="importPaper">导入底卷图像 =></span>
      </div>
    </div>
  </div>
</template>
