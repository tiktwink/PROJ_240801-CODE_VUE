<script setup>
  ...
  import ExportBaseinfo from "@/view/pages/utility/ExportBaseinfo.vue";
  ...
  watch([() => exportInfo.orgId], ([newOrgId], [oldOrgId]) => {
	//orgId
  ...
  const queryOrg = useDebounceFn((id) => {
	axiosUtil.post('/api/org/search', {orgId: id})
		.then(res => {
		  ...
		  if (res.data.code === 0) {
			errorFlag = false
			exportInfo.orgName = res.data.data[0].orgName; //注意接口返回的是列表
		  } else {
			toast.add({
			  severity: 'error',
  ...
  const handleSubmit = () => {
	if (errorFlag || imList.value.length === 0 || exportInfo.orgId === '' || exportInfo.cangkuName === '') {
	  toast.add({
		severity: 'warn',
		detail: '请检查是否遗漏必要信息',
		life: 2000
	...
	axiosUtil.post("/api/export/add", {
	  ...exportInfo,
	  exportContent: imList.value.filter(im => im.count > 0)
	})
		.then(res => {
		  if (res.data.code === 0) {
			toast.add({
			...
			exportInfo.exportId = res.data.data
		  } else {
			toast.add({
			  severity: 'error',
			  detail: '出库单上传失败',
  ...
  const computeTotalCostAfterOff = computed(() => computeTotalCost.value * (1 - exportInfo.offRate / 100))

</script>

<template>
  <div class="w-full h-full">
	<ExportBaseinfo v-model:baseinfo="exportInfo" />
	...
	<div>
	  <div class="titles flex items-center w-full px-8 py-2" style="height:fit-content;">
		...
		<span><text class="label">出库数量</text></span>
		<span><text class="label">出库单价(元)</text></span>
		<span><text class="label">出库金额(元)</text></span>
		<span><text class="label">备注</text></span>
	  </div>
	  
	  <div class="w-full px-8 mb-4">
		
		<div v-for="(im,index) in imList" class="items  flex items-center w-full first:border-t border-b border-gray-300">
		  <span style="width:40px; flex:none; margin-right:0.5rem;"> <n-checkbox v-model:checked="im.selected" /></span>
		  <span style="width:50px; flex:none; margin-right:0.5rem; ">
            <text style="font-size:0.7rem;font-weight:bold;width:24px;height:23px; border-radius:50%;background-color:#f59e0c;color:white; display:flex;justify-content:center;align-items:center;">{{
				index + 1
			  }}</text>
          </span>
	  ...
	  <div class="w-full flex justify-center items-center gap-2">
		<span class="px-3 py-1 rounded-2xl bg-amber-200 text-gray-500 hover:text-white hover:bg-amber-400 cursor-pointer " @click.stop="handleAddIm">新增出库条目</span>
		...
		<span v-if="imList.length>0" class="px-3 py-1 rounded-2xl bg-amber-200 text-gray-500 hover:text-white hover:bg-amber-400 cursor-pointer flex items-center" @click.stop="handleSubmit"><text>提交出库单</text><text class="text-[0.7rem] font-bold">(所有条目)</text></span>
	  </div>
	  ...
	  <div v-show="imList.length>0" class=" mx-8 my-2 flex w-fit items-center justify-start gap-3 text-[0.8rem] bg-gray-200 px-2.5 py-1 rounded-lg">
		...
		<span class="flex items-center gap-1"><text>折扣比例(省略%):</text><input class=" max-w-[32px] pt-[1px] outline-none border cursor-default text-center" v-model.number="exportInfo.offRate" /></span>
